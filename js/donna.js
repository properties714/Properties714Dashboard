/* Donna v3 + Context Switcher — Properties 714 */
(function(){
  ...
  var CSS='.d-fab{position:fixed;bottom:22px;right:22px;z-index:9000;width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#0D9488,#0F766E);box-shadow:0 4px 18px rgba(13,148,136,.55);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:transform .2s,box-shadow .2s;font-family:Syne,sans-serif;}'+
  '.d-fab:hover{transform:scale(1.1);box-shadow:0 6px 26px rgba(13,148,136,.65);}'+
  '.d-fab-lbl{font-weight:800;font-size:20px;color:#fff;line-height:1;pointer-events:none;}'+
  ...
  '.d-row{display:flex;gap:7px;align-items:flex-end;}.d-row.me{flex-direction:row-reverse;}'+
  ...
  '.d-av.ai{background:linear-gradient(135deg,#0D9488,#0F766E);color:#fff;font-family:Syne,sans-serif;}.d-av.me{background:#EFF6FF;color:#2563EB;}'+
  ...
  /* Context switcher */
  '.ctx-bar{display:none;align-items:center;gap:8px;padding:8px 10px;margin:6px 8px;border-radius:8px;background:rgba(124,58,237,.15);border:1px solid rgba(124,58,237,.25);}'+
  '.ctx-bar.show{display:flex;}'+
  '.ctx-bar-txt{flex:1;font-size:11px;font-weight:600;color:#C4B5FD;}'+
  '.ctx-btn{padding:4px 10px;border-radius:6px;font-size:10px;font-weight:700;cursor:pointer;border:none;font-family:Inter,sans-serif;transition:all .12s;}'+
  '.ctx-btn-sa{background:rgba(124,58,237,.3);color:#C4B5FD;}.ctx-btn-sa:hover{background:rgba(124,58,237,.5);}'+
  '.ctx-btn-org{background:rgba(37,99,235,.25);color:#93C5FD;}.ctx-btn-org:hover{background:rgba(37,99,235,.4);}'+
  ...
  function injectContextSwitcher(){
    if(!window.P714Auth||!P714Auth.isSuperAdmin)return;
    // Wait a tick for auth to be ready
    setTimeout(function(){
      if(!P714Auth.isSuperAdmin||!P714Auth.isSuperAdmin())return;
      var nav=document.querySelector('.sb-nav');
      if(!nav)return;
      if(document.getElementById('ctx-switcher'))return;
      var isSAPage=location.pathname.startsWith('/super-admin/');
      var bar=document.createElement('div');
      bar.id='ctx-switcher';
      bar.className='ctx-bar show';
      if(isSAPage){
        bar.innerHTML='<span style="font-size:14px">⚡</span><span class="ctx-bar-txt">Super Admin</span><button class="ctx-btn ctx-btn-org" onclick="location.href=\'/\'">🎯 Mi Org</button>';
      }else{
        bar.innerHTML='<span style="font-size:14px">Σ</span><span class="ctx-bar-txt">'+P714Auth.getOrganization()?.name+'</span><button class="ctx-btn ctx-btn-sa" onclick="location.href=\'/super-admin/\'">Super Admin →</button>';
      }
  ...
  function boot(){
    ...
    win.innerHTML=[
      '<div class="d-hdr"><div class="d-hdr-av">D</div><div class="d-hdr-info"><div class="d-hdr-name">Donna</div><div class="d-hdr-st"><div class="d-hdr-dot"></div>Asistente de Properties 714</div></div><button class="d-x" id="d-close">&#215;</button></div>',
      '<div class="d-body" id="d-body"><div class="d-row"><div class="d-av ai">D</div><div class="d-bub ai">\u00a1Hola! Soy <strong>Donna</strong> \ud83c\udfe0<br><br>Estoy aqu\u00ed para ayudarte con deals, MAO, estrategias, o cualquier duda del sistema.<br><br>\u00bfEn qu\u00e9 te puedo ayudar?</div></div></div>',
      '<div class="d-qs" id="d-qs"><button class="d-q" id="q1">\ud83d\udcd0 C\u00f3mo calcular MAO</button><button class="d-q" id="q2">\ud83c\udfe1 Flip vs Rental</button><button class="d-q" id="q3">\ud83d\udcca Qu\u00e9 es el score</button><button class="d-q" id="q4">\ud83d\udccd Qu\u00e9 es ARV</button></div>',
      '<div class="d-foot"><textarea class="d-inp" id="d-inp" rows="1" placeholder="Escribe tu pregunta..."></textarea><button class="d-send" id="d-send">&#10148;</button></div>'
    ].join('');
  ...
  function toggle(){open=!open;var w=document.getElementById('d-win');if(w)w.classList.toggle('show',open);if(open)setTimeout(function(){var i=document.getElementById('d-inp');if(i)i.focus();},240);}
  function quick(txt){var i=document.getElementById('d-inp');if(i){i.value=txt;send();}}
  ...
  function send(){
    ...
    var qs=document.getElementById('d-qs');if(qs)qs.style.display='none';
    addBub('me',txt);
    ...
    .catch(function(e){msgs.pop();addBub('ai','\u274c Error: '+e.message);})
    .finally(function(){setBusy(false);});
  }

  function addBub(role,txt){
    var body=document.getElementById('d-body');if(!body)return;
    body.querySelector('.d-typing-row')&&body.querySelector('.d-typing-row').remove();
    var isAI=role==='ai';
    var row=document.createElement('div');row.className='d-row'+(isAI?'':' me');
    var fmt=txt.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')
  ...
  function setBusy(v){
    busy=v;
    var btn=document.getElementById('d-send');if(btn)btn.disabled=v;
    var body=document.getElementById('d-body');if(!body)return;
    if(v){
      var r=document.createElement('div');r.className='d-row d-typing-row';
